...
lint: ## lint code
	...
	xenon --max-absolute C --max-modules B --max-average A *.py --exclude addon_updater.py,addon_updater_ops.py,search.py
	mypy ../hana3d | grep '../hana3d/src/' && exit 1 || exit 0


test: ## test code
	HANA3D_ENV=$(STAGE) blender -b -P tests/install.py -noaudio
	HANA3D_ENV=$(STAGE) blender -b -P tests/credentials.py -noaudio
