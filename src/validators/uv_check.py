...
def get_multiple_uv_models(models: List[str]) -> List[str]:
    multiple_uv_models = [
        model for model in models
        if (
            bpy.data.objects[model].data is not None
            and hasattr(bpy.data.objects[model].data, 'uv_layers')
            and len(bpy.data.objects[model].data.uv_layers) > 1  # noqa: WPS219
        )
    ]
    return multiple_uv_models
...
def fix_uv_layers():
    ...
    multiple_uv_models = get_multiple_uv_models(models)
    for model in multiple_uv_models:
        uv_layers = bpy.data.objects[model].data.uv_layers
        unwanted_uvs = [
            uv for uv in uv_layers
            if uv != bpy.data.objects[model].data.uv_layers.active
        ]
        while unwanted_uvs:
