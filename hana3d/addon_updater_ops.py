...
def updater_run_install_popup_handler(scene):
    ...
    # elif not isinstance(updater.update_version, tuple):
...
def register(bl_info):
    ...
    updater.overwrite_patterns = ['*.yml', '*.png', '*.jpg', 'README.md', 'LICENSE.txt']
    # updater.overwrite_patterns = []
    # other examples:
    # ["*"] means ALL files/folders will be overwritten by update,
    #  was the behavior pre updater v1.0.4
    # [] or ["*.py","*.pyc"] matches default blender behavior,
    # ie same effect if user installs update manually without deleting the existing addon first
    #    e.g. if existing install and update both have a resource.blend file,
    # the existing installed one will remain
    # ["some.py"] means if some.py is found in addon update,
    # it will overwrite any existing some.py in current addon install, if any
    # ["*.json"] means all json files found in addon update will overwrite
    # those of same name in current install
    # ["*.png","README.md","LICENSE.txt"] means the readme, license,
    # and all pngs will be overwritten by update
    ...
    updater.include_branch_list = ['dev']  # None is the equivalent to setting ['master']
