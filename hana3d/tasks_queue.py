...
def queue_worker():
    ...
    stashed = {}
    # first round we get all tasks that are supposed to be
    #  stashed and run only once (only_last option)
    # stashing finds tasks with the property only_last
    # and same command and executes only the last one.
    while not q.empty():
        task = q.get()
        if task.only_last:
